<?php

/* @var $this yii\web\View */

use yii\helpers\Html;
use yii\helpers\Url;

$this->title = 'Як це працює';
$this->params['breadcrumbs'][] = $this->title;


$this->registerJsFile('http://maps.googleapis.com/maps/api/js?key=AIzaSyC9CXLB6tTD94qL3Jdxbesrx9Cj6fUUumE&amp;sensor=false&amp;language=en', ['depends' => [\yii\web\JqueryAsset::className()]]);
$this->registerJsFile('/js/map.js', ['depends' => 'yii\web\JqueryAsset']);
?>

<div class="tt-header-margin"></div>
<!-- TT-BANNER TYPE-6 -->
<div class="tt-banner type-6 wth-swicher background-block" style="background-image:url(/img/imgblock/banner_18.jpg);">
    <div class="container">
        <div class="tt-swicher">
            <a href="<?=Url::to(['/site/howitwork'])?>">Замовник</a>
            <a class="active" href="<?=Url::to(['/site/howitworkmaster'])?>">Виконавець</a>
        </div>
        <h2 class="tt-banner-title h2 small light">Отримуйте замовлення на пряму</h2>
        <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <div class="simple-text size-4 light">
                    <p>Негайте часу і додавайте свою компанію до списку таких самих професіоналів як! Пошук клієнтів, ще ніколи не був такий простий!</p>
                </div>
            </div>
        </div>
    </div>
</div>

<div class="container">
    <div class="empty-space marg-sm-b40 marg-lg-b85"></div>
    <h3 class="h3 text-center">Як отримувати замовлення на “Всі майстри”:</h3>
    <div class="empty-space marg-sm-b30 marg-lg-b45"></div>
    <!-- TT-SERVICE TYPE-3 -->
    <div class="row">
        <div class="col-sm-4">
            <div class="tt-service type-3 grey">
                <div class="tt-service-entry">
                    <div class="tt-service-count">1</div>
                    <div class="tt-service-icon">
                        <img src="data:image/png;base64," alt="">
                    </div>
                    <a class="tt-service-title h5" href="master-register.html">Зареєструйтесь на сайті</a>
                    <div class="simple-text size-3">
                        <p>Дайте відповідь на кілька питань, щоб ми могли підібрати вам майстрів.</p>
                    </div>
                    <div class="tt-service-arrow"><img src="data:image/png;base64," alt=""></div>
                </div>
            </div>
            <div class="empty-space marg-sm-b60"></div>
        </div>
        <div class="col-sm-4">
            <div class="tt-service type-3 grey">
                <div class="tt-service-entry">
                    <div class="tt-service-count">2</div>
                    <div class="tt-service-icon">
                        <img src="data:image/png;base64," alt="">
                    </div>
                    <a class="tt-service-title h5" href="professionals-list.html">Контактуйте із замовниками</a>
                    <div class="simple-text size-3">
                        <p>Обговорюйте особливості, які терміни, ціноутворення і обсяг робіт.</p>
                    </div>
                    <div class="tt-service-arrow"><img src="data:image/png;base64," alt=""></div>
                </div>
            </div>
            <div class="empty-space marg-sm-b60"></div>
        </div>
        <div class="col-sm-4">
            <div class="tt-service type-3 grey">
                <div class="tt-service-entry">
                    <div class="tt-service-count">3</div>
                    <div class="tt-service-icon">
                        <img src="data:image/png;base64," alt="">
                    </div>
                    <a class="tt-service-title h5" href="professionals-detail.html">Отримуйте завдання</a>
                    <div class="simple-text size-3">
                        <p>Допоможіть іншим замовникам зробити правельний вибір</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="empty-space marg-sm-b50 marg-lg-b90"></div>
</div>
<div class="tt-bg-grey">
    <div class="tt-devider"></div>
    <div class="container">
        <div class="empty-space marg-sm-b40 marg-lg-b75"></div>
        <h3 class="h3 text-center">Скільки коштують послуги сайту</h3>
        <div class="empty-space marg-lg-b20"></div>
        <div class="simple-text size-3">
            <p>In hac habitasse platea dictumst. Integer bibendum sollicitudin elit id pellentesque. Vivamus non tortor auctor mi elementum dapibus. Sed rutrum, mauris vel pulvinar bibendum, enim ligula tincidunt leo, a ornare enim tortor a lorem. Vestibulum lobortis condimentum ligula at bibendum. Fusce ultricies, purus sed fringilla bibendum, mi nulla elementum nisi, vitae sollicitudin enim lorem in nulla.</p>

            <p>Donec facilisis nibh non libero suscipit vulputate. Aenean mollis massa quam, nec vehicula leo porttitor nec.</p>
        </div>
        <div class="empty-space marg-sm-b30 marg-lg-b35"></div>


        <?=\common\widgets\PriceTable::widget()?>

        <div class="empty-space marg-sm-b40 marg-lg-b85"></div>
    </div>
    <div class="tt-devider"></div>
</div>

<div class="container">
    <div class="empty-space marg-sm-b50 marg-lg-b100"></div>

    <!-- TT-IMGBOX -->
    <div class="row vertical-middle">
        <div class="col-sm-5 col-sm-push-7">
            <div id="map-canvas" class="style-1 tt-imgbox-map" data-lat="49.840576" data-lng="24.032141" data-zoom="11"></div>
            <div class="addresses-block">
                <a class="marker" data-lat="49.860892" data-lng="24.038072" data-marker="/img/map/marker.png"  data-string="<b>1. Here is some address</b><br> or email or phone or something else..."></a>
                <a class="marker" data-lat="49.841533" data-lng="24.065210" data-marker="/img/map/marker.png"  data-string="<b>1. Here is some address</b><br> or email or phone or something else..."></a>
                <a class="marker" data-lat="49.839671" data-lng="23.995056" data-marker="/img/map/marker.png"  data-string="<b>1. Here is some address</b><br> or email or phone or something else..."></a>
                <a class="marker" data-lat="49.819372" data-lng="24.048176" data-marker="/img/map/marker.png"  data-string="<b>1. Here is some address</b><br> or email or phone or something else..."></a>
                <a class="marker" data-lat="49.891032" data-lng="23.967629" data-marker="/img/map/marker_2.png"  data-string="<b>1. Here is some address</b><br> or email or phone or something else..."></a>
                <a class="marker" data-lat="49.806331" data-lng="23.946266" data-marker="/img/map/marker_2.png"  data-string="<b>1. Here is some address</b><br> or email or phone or something else..."></a>
                <a class="marker" data-lat="49.787293" data-lng="24.127074" data-marker="/img/map/marker_2.png"  data-string="<b>1. Here is some address</b><br> or email or phone or something else..."></a>
            </div>
        </div>
        <div class="col-sm-7 col-sm-pull-5">
            <div class="tt-imgbox-info style-2">
                <a class="tt-imgbox-title h4" href="blog-detail.html">Обирайте роботу, де вам зручно</a>
                <div class="simple-text size-3">
                    <p>Nunc metus mauris, semper sit amet varius ut, posuere in ligula. Maecenas sollicitudin ullamcorper interdum. Duis consequat ut libero ut varius. Nulla augue ligula, fringilla in pharetra at, mollis ut metus.</p>
                </div>
            </div>
        </div>
    </div>
    <div class="empty-space marg-sm-b30 marg-lg-b50"></div>
    <div class="row vertical-middle">
        <div class="col-sm-5">
            <img class="tt-imgbox-img center-block img-responsive" src="/img/imgbox/imgbox_2.jpg" alt="">
        </div>
        <div class="col-sm-7">
            <div class="tt-imgbox-info style-2">
                <a class="tt-imgbox-title h4" href="blog-detail.html">Налаштовуйте сповіщення про роботу, яка вас зацікавить</a>
                <div class="simple-text size-3">
                    <p>Nunc metus mauris, semper sit amet varius ut, posuere in ligula. Maecenas sollicitudin ullamcorper interdum. Duis consequat ut libero ut varius. Nulla augue ligula, fringilla in pharetra at, mollis ut metus.</p>
                </div>
            </div>
        </div>
    </div>
    <div class="empty-space marg-sm-b30 marg-lg-b50"></div>
    <div class="row vertical-middle">
        <div class="col-sm-5 col-sm-push-7">
            <img class="tt-imgbox-img center-block img-responsive" src="/img/imgbox/imgbox_3.jpg" alt="">
        </div>
        <div class="col-sm-7 col-sm-pull-5">
            <div class="tt-imgbox-info style-2">
                <a class="tt-imgbox-title h4" href="blog-detail.html">Шукайте нові, прибуткові завдання</a>
                <div class="simple-text size-3">
                    <p>Nunc metus mauris, semper sit amet varius ut, posuere in ligula. Maecenas sollicitudin ullamcorper interdum. Duis consequat ut libero ut varius. Nulla augue ligula, fringilla in pharetra at, mollis ut metus.</p>
                </div>
            </div>
        </div>
    </div>
    <div class="empty-space marg-sm-b40 marg-lg-b70"></div>
</div>
<div class="tt-devider"></div>

<div class="tt-bg-grey">
    <div class="container">
        <div class="empty-space marg-sm-b40 marg-lg-b75"></div>
        <h3 class="h3 text-center">Часті питання по роботі на “ВсіМайстри”</h3>
        <div class="empty-space marg-lg-b30"></div>

        <?=\common\widgets\Faq::widget(array('id' => '2')) ?>

        <div class="empty-space marg-sm-b40 marg-lg-b90"></div>
    </div>
    <div class="tt-devider"></div>
</div>

<div class="container">
    <div class="empty-space marg-sm-b30 marg-lg-b60"></div>
    <h2 class="h2 text-center">Готові працювати із ВсіМайстри?</h2>
    <div class="empty-space marg-lg-b30"></div>
    <div class="text-center">
        <a href="master-register.html" class="button type-1 size-4 color-3 uppercase">зареєструватись зараз</a>
    </div>
    <div class="empty-space marg-sm-b40 marg-lg-b80"></div>
</div>
<div class="tt-devider"></div>
