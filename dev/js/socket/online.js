$(document).ready(function(){
    ...
    socket.on('msg', function(data) {
        ...
        $('div.tt-messages-user[data-id="'+data.suggestion_id+'"][data-support="'+data.support+'"]').find('div.simple-text p').html(data.msg);
    ...
    $(document).on("beforeSubmit", "form#send_msg", function(e) {
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                if (data.status==1){
                    socket.emit('msg', {member: $('header.tt-header.user').data('id'), suggestion_id: $('div.tt-messages-user.clearfix.active').data('id'), support:$('div.tt-messages-user.clearfix.active').data('support'), msg: data.msg, id_unread:data.id_unread});

                    $('#membermsg-msg').val('');
                    $('.tt-messages-text').append('<div class="tt-messages-item other simple-text"><p>'+data.msg+'</p></div>');
                    $(".tt-messages-text").animate({ scrollTop: $('.tt-messages-text').prop("scrollHeight")}, 500);
                    $('div.tt-messages-user[data-id="'+$('div.tt-messages-user.clearfix.active').data('id')+'"][data-support="'+$('div.tt-messages-user.clearfix.active').data('support')+'"]').find('div.simple-text p').html(data.msg);
                }
            }
        });
